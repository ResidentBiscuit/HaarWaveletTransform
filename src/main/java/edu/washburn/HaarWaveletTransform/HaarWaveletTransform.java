...
{
	public static double[] discreteHaarWaveletTransform(double[] input, int resolution) {
		if(resolution < 1) {
			return input;
		}
	    ...
	    for (int length = input.length >> 1, iteration = 1; ; length >>= 1) {
	        ...
	        if (length == 1 || iteration == resolution) {
	            return output;
	        }
